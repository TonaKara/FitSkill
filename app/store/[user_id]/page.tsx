...
export async function generateMetadata({
  ...
  const resolvedCanonicalPath = `/store/${encodeURIComponent(canonicalSegment)}`
...
export default async function Page({ params, searchParams }: StorePageProps) {
  ...
  if (profile?.custom_id?.trim() && isUuid(identifier)) {
    permanentRedirect(`/store/${encodeURIComponent(profile.custom_id.trim())}`)
  }
  return <StorePage resolvedProfileId={profile?.id} />
}
