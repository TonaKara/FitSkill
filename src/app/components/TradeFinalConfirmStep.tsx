...
import {
  resolveTradeLegalBullets,
  TRADE_LEGAL_BULLETS_BUYER,
  TRADE_LEGAL_BULLETS_SELLER,
} from "@/lib/trade-legal-notices"
...
export function TradeFinalConfirmStep({
  ...
  const t = useTranslations("tradeFinalConfirm")
  const locale = useLocale()
  const [agreed, setAgreed] = useState(false)
  const [legalModal, setLegalModal] = useState<LegalDocumentKind | null>(null)
  const bulletsSource = variant === "seller" ? TRADE_LEGAL_BULLETS_SELLER : TRADE_LEGAL_BULLETS_BUYER
  const bullets = resolveTradeLegalBullets(bulletsSource, locale)
  const physicalSafetyNotice = variant === "buyer" ? t("physicalSafetyNotice") : null
          ...
          <ul className="list-disc space-y-2 pl-5 text-sm leading-relaxed text-foreground">
            {physicalSafetyNotice ? (
              <li className="list-none rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-amber-950 dark:border-amber-500/40 dark:bg-amber-950/30 dark:text-amber-100">
                <span className="font-semibold">{t("physicalSafetyHeading")}</span>
                <br />
                {physicalSafetyNotice}
              </li>
            ) : null}
