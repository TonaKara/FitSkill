-- claim_skill_application_after_payment (bigint 版) を currency 伝達対応に再作成。
--
-- # 変更点
-- 1. skills SELECT に s.currency を追加
-- 2. transactions INSERT に currency = v_skill.currency を追加
--
-- # 既存ユーザーへの影響
-- - skills.currency は NOT NULL DEFAULT 'JPY'。すべての既存行は 'JPY'。
-- - transactions.currency は NOT NULL DEFAULT 'JPY'。新しい INSERT で明示渡しになるが
--   渡す値は v_skill.currency なので、既存 JPY 行については従来挙動と完全同等。
-- - 関数シグネチャ（引数・戻り値）は不変。
--
-- # 旧定義との差分（diff）
--   select s.id, s.max_capacity, s.user_id, s.price
-- → select s.id, s.max_capacity, s.user_id, s.price, s.currency
--
--   insert into public.transactions as tr (skill_id, buyer_id, seller_id, price, "status", stripe_payment_intent_id)
-- → insert into public.transactions as tr (skill_id, buyer_id, seller_id, price, currency, "status", stripe_payment_intent_id)
--
--   values (p_skill_id, p_buyer_id, p_seller_id, greatest(0, round(v_skill.price::numeric)::integer), 'active', ...)
-- → values (p_skill_id, p_buyer_id, p_seller_id, greatest(0, round(v_skill.price::numeric)::integer), coalesce(v_skill.currency, 'JPY'), 'active', ...)
...
begin
  ...
  if nullif(trim(p_stripe_checkout_session_id), '') is not null then
    select r.id, r.skill_id, r.buyer_id, r.seller_id, r.expires_at, r.consumed_at, r.released_at, r.stripe_checkout_session_id
      into v_reservation
    ...
    if found then
      ...
      if v_existing.status = 'awaiting_payment' then
        ...
        update public.transactions tr
           set "status" = 'active',
               stripe_payment_intent_id = coalesce(nullif(trim(p_stripe_payment_intent_id), ''), tr.stripe_payment_intent_id),
               completed_at = null,
               auto_complete_at = null
         where tr.id = v_existing.id
           and tr."status" = 'awaiting_payment'
         returning tr.id, tr."status" into v_tx_id, v_tx_status;
  ...
  begin
    insert into public.transactions as tr (
      skill_id,
      buyer_id,
      seller_id,
      price,
