-- claim_skill_application_after_payment:
-- 1) Checkout 予約の consumed_at は取引が確定して返す直前に付与（INSERT/UPDATE 成功後と冪等リターン時）。
-- 2) unique_violation 時は PI が無くても skill+buyer+seller で既存行を拾い返す（Webhook / フロントの二重呼び出し）。
-- count_active_skill_checkout_reservations は従来どおり consumed_at is null で除外（変更なし）。
